...
var LexWebUiIframe = function (document, window, defaultOptions) {
  ...
  */
  ...
  function BotLoader(optionsParam) {
    ...
    this.config = {
      ...
      aws: {
        cognitoPoolId: 'us-east-1:545dd4bc-fb45-4164-87f9-7e0d71572706'
      },
      iframeConfig: {
        lex: { botName: 'BMSBOT' }
      }
  ...
  BotLoader.prototype.init = function (configParam) {
    ...
    Promise.resolve().then(function initConfig() {
      return Promise.resolve().then(function initConfigFromJsonFile() {
        return self.options.shouldLoadConfigFromJsonFile ? loadConfigFromJsonFile(self.options.configUrl) : Promise.resolve(self.config);
      }).then(function initConfigFromEvent(config) {
        return self.options.shouldLoadConfigFromEvent ? loadConfigFromEvent(config) : Promise.resolve(config);
      ...
      return addContainer(self.options.containerClass).then(function assignContainer(containerElement) {
        self.containerElement = containerElement;
        return containerElement;
      });
    }).then(function initSdk() {
      return self.options.shouldAddAwsSdk ? addAwsSdk(self.containerElement, self.options.containerClass, self.options.sdkUrl) : Promise.resolve();
    }).then(function initCredentials() {
      return createCredentials(self.config.aws.region || 'us-east-1', self.config.aws.cognitoPoolId).then(function assignCredentials(credentials) {
        self.credentials = credentials;
      });
    }).then(function addMessageFromIframeListener() {
      window.addEventListener('message', self.onMessageFromIframe.bind(self), false);
      ...
      return addIframe(self.config.iframeOrigin, self.containerElement, self.options.containerClass, self.options.iframeSrcPath).then(function assignIframe(iframeElement) {
        self.iframeElement = iframeElement;
      }).then(function waitForChatBot() {
        return self.waitForChatBotReady();
      });
    }).then(function displayIframe() {
      return Promise.resolve().then(function minimizeUi() {
        return self.config.loadIframeMinimized ? self.sendMessageToIframe({ event: 'toggleMinimizeUi' }) : Promise.resolve();
      }).then(function showUi() {
        toggleShowUi(self.containerElement, self.options.containerClass);
      });
    }).then(function setupEvents() {
      return Promise.resolve().then(function addParentToIframeListener() {
        document.addEventListener('lexWebUiMessage', self.onMessageToIframe.bind(self), false);
  ...
  **************************************************************************/
  ...
  function mergeConfig(baseConfig, srcConfig) {
    ...
    return Object.keys(baseConfig).map(function (key) {
      ...
      if (key in srcConfig && !isEmpty(srcConfig[key])) {
        value = _typeof(baseConfig[key]) === 'object' ?
        // recursively merge sub-objects in both directions
        _extends(mergeConfig(srcConfig[key], baseConfig[key]), mergeConfig(baseConfig[key], srcConfig[key])) : srcConfig[key];
      }
      mergedConfig[key] = value;
      return mergedConfig;
    }).reduce(function (merged, configItem) {
      return _extends({}, merged, configItem);
    }, {});
  }
  ...
  function addAwsSdk(divElement, containerClass, sdkUrl) {
    return new Promise(function addAwsSdkPromise(resolve, reject) {
      ...
      var sdkScriptElement = document.querySelector('.' + containerClass + ' script');
  ...
  function createCredentials(region, cognitoPoolId) {
    ...
    var credentials = new AWS.CognitoIdentityCredentials({ IdentityPoolId: cognitoPoolId }, { region: region });
  ...
  function addIframe(origin, divElement, containerClass, iframeSrcPath) {
    var iframeElement = document.querySelector('.' + containerClass + ' iframe');
    if (iframeElement) {
  ...
  **************************************************************************/
  ...
  function toggleShowUi(containerElement, containerClass) {
    containerElement.classList.toggle(containerClass + '--show');
  }
  ...
  function toggleMinimizeUi(containerElement, containerClass) {
    containerElement.classList.toggle(containerClass + '--minimize');
  }
  ...
  **************************************************************************/
  ...
  BotLoader.prototype.getCredentials = function () {
    ...
    return self.credentials.getPromise().then(function storeIdentityId() {
  ...
  BotLoader.prototype.sendMessageToIframe = function (message) {
    var self = this;
    if (!self.iframeElement || !('contentWindow' in self.iframeElement) || !('postMessage' in self.iframeElement.contentWindow)) {
      return Promise.reject('invalid iframe element');
    }

    return new Promise(function sendMessageToIframePromise(resolve, reject) {
      var messageChannel = new MessageChannel();
      messageChannel.port1.onmessage = function sendMessageToIframeResolver(evt) {
      ...
      self.iframeElement.contentWindow.postMessage(message, self.config.iframeOrigin, [messageChannel.port2]);
    });
  };
  ...
  BotLoader.prototype.onMessageToIframe = function (evt) {
    ...
    if (!evt || !('detail' in evt) || !evt.detail || !('message' in evt.detail)) {
      console.error('malformed message to iframe event: ' + JSON.stringify(evt));
      return;
    }
  ...
  BotLoader.prototype.createIframeMessageHandlers = function () {
    ...
    return {
      ...
      onGetCredentials: function onGetCredentials(evt) {
        return self.getCredentials().then(function resolveGetCredentials(creds) {
...
var lexWebUi = function (document, window, BotLoader) {
  ...
  function isSupported() {
    var features = ['Audio', 'Blob', 'MessageChannel', 'Promise', 'URL', 'localStorage', 'postMessage'];
  ...
  if (isSupported()) {
    ...
    document.addEventListener('DOMContentLoaded', function initBoloader() {
      botLoader.init();
    }.bind(this), false);
    return botLoader;
  } else {
    console.warn('chatbot UI could not be be loaded - ' + 'could not find require browser functions');
  }
}(document, window, LexWebUiIframe);

//# sourceMappingURL=chatbot-ui-iframe-loader.js.map