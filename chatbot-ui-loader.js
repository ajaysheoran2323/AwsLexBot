...
var LexWebUiLoader = function createLexWebUiLoader(document, window, defaultOptions) {
    ...
    */
    return {
      script: [{
        ...
        name: 'LexWebUi',
        url: getMinUrl('./lex-web-ui.js', options.shouldLoadMinDeps)
      }],
      css: [{
        ...
        name: 'vuetify',
        url: getMinUrl('https://unpkg.com/vuetify@0.16.9/dist/vuetify.css', options.shouldLoadMinDeps)
      }, {
        name: 'lex-web-ui',
        url: getMinUrl('./lex-web-ui.css', options.shouldLoadMinDeps)
      }, {
  ...
  function loadDependencies(options) {
    if (!('dependencies' in options) || !('css' in options.dependencies) || !('script' in options.dependencies)) {
  ...
  function loadConfig(options, configParam) {
    ...
    if (options.shouldIgnoreConfigWhenEmbedded && url.indexOf('lexWebUiEmbed=true') !== -1) {
      return Promise.resolve({});
    }

    return Promise.resolve().then(function () {
      return options.shouldLoadConfigFromJsonFile ? loadConfigFromJsonFile(options.configUrl) : Promise.resolve({});
    }).then(function (configJson) {
      return options.shouldLoadConfigFromMobileHubFile ? mergeMobileHubConfig(configJson) : Promise.resolve(configJson);
    }).then(function (configMerged) {
      return _extends(configMerged, configParam);
    });
  }
  ...
  function mergeMobileHubConfig(config) {
    ...
    return new Promise(function mergeMobileHubConfigPromise(resolve, reject) {
      ...
      botName = botName || (config.lex && config.lex.botName ? config.lex.botName : '');

      var mergedConfig = _extends({ cognito: {}, lex: {} }, config);
...
//# sourceMappingURL=chatbot-ui-loader.js.map